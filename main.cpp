...
int main(void) {
  ...
  Shader shader("shader/vertex.vs", "shader/fragment.fs", nullptr,
                "shader/tessControl.tcs", "shader/tessEval.tes");

  // it needs to be allocated dynamically in order to be destroy before
  // glfwTerminate
  Terrain *terrain = new Terrain("assets/iceland_heightmap.png", &shader);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glDrawArrays(GL_PATCHES, 0,
                 terrain->NUM_PATCH_PTS * terrain->rez * terrain->rez);
